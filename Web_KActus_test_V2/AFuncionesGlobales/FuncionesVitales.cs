...
namespace Web_Kactus_Test_V2
{
    public class FuncionesVitales 
    {
        ...
        static public void LimpiarProcesos()
        {
            ...
            Process[] processes4 = Process.GetProcessesByName("chrome");
            if (processes4.Length > 0)
            {
                for (int i = 0; i < processes4.Length; i++)
                {
                    processes4[i].Kill();
                }
            }
        ...
        static public void KillProcesos(string proceso)
        {
            System.Diagnostics.Process.Start("cmd.exe", "/c taskkill /IM "+ proceso);
